...
> **Ported on merge (2026-07-28).** This plan was written against the
> pre-Postgres, pre-Gamma codebase and is kept as the historical design record.
> It was implemented on the `mvp` trunk instead, where:
> - the DAL is **psycopg against Postgres**, not `sqlite3` — placeholders are
>   `%s`, not `?`, and connections are `psycopg.Connection`, not
>   `sqlite3.Connection`;
> - `WHERE CATEGORY = ? COLLATE NOCASE` becomes
>   `WHERE LOWER(CATEGORY) = LOWER(%s)` (with the value stripped; blank means
>   no filter);
> - the `DISTINCT` category listing cannot `ORDER BY` an expression outside its
>   select list, so it uses a subquery/`GROUP BY` form ordered
>   `ORDER BY LOWER(c), c`;
> - test fixtures are `fresh_test_conn()` / `fresh_test_db()` against the
>   Postgres test database, not an in-memory sqlite connection;
> - `GET /events` returns a bare `list[GammaEvent]`, **not** the
>   `{events, total, limit, offset}` envelope — acceptance criteria phrased as
>   `resp.total == 1` now read `len(body) == 1`, and `body["events"][0]["event"]["slug"]`
>   reads `body[0]["slug"]`;
> - `list_events_with_markets` keeps `mvp`'s ordering
>   `ORDER BY VOLUME_24HR DESC NULLS LAST, EVENT_ID DESC` (not `EVENT_ID DESC`),
>   guarded by `tests/test_event_volume.py`.
>
> Everything else — the tag-derived category resolver, `include_tag=true`, the
> strictest-category-wins sync rule — shipped as written.
