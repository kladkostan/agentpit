...
- **Does not run the production migration.** `scripts/migrate_users_to_workos.py` is no longer a prerequisite: `_link_existing_account` adopts an unmigrated row by email on its owner's first sign-in, and task 7 measured that against a real row. If you run it anyway, run it **after** the api container is up — never before. It opens the database with `create_tables=False` (deliberately: it is a second writer against a live API) and its first statement selects `WORKOS_USER_ID`, a column that does not exist until the new image has started and `TableCreate._migrate_users_table` has added it. Run before the deploy, it fails on the very first query.
