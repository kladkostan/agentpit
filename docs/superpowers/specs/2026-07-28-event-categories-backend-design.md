...
> **Ported on merge (2026-07-28).** Written against the pre-Postgres,
> pre-Gamma codebase; kept as the historical design record. As shipped on the
> `mvp` trunk: the DAL is **psycopg/Postgres** (`%s` placeholders,
> `psycopg.Connection`), `WHERE CATEGORY = ? COLLATE NOCASE` is
> `WHERE LOWER(CATEGORY) = LOWER(%s)` on a stripped value, `list_event_categories`
> did **need** a change (Postgres rejects `SELECT DISTINCT … ORDER BY <expr not in
> select list>`, so it uses a subquery/`GROUP BY` ordered `ORDER BY LOWER(c), c`),
> and the wire shape of `GET /events` is a bare `list[GammaEvent]` — there is no
> `{events, total}` envelope, so `total` reads as `len(body)`. `GET /events/categories`
> keeps its own `ListEventCategoriesResponse` model and is unaffected.
