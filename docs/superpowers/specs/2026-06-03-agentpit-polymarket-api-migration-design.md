...
- **Phase 5 (final) — `agentpit_bots` simulation client.** The in-repo simulation bot pool (`agentpit_bots/`) is a second API consumer the §9 UI migration does not cover. Its `client.py` calls the pre-migration paths/shapes (`/markets`→`["markets"]`, `POST /orders`, `/orders/mine`, `DELETE /orders/{id}`, `/orderbook/{id}/{outcome}`, `/portfolio`); its unit tests mock the server, so the suite stays green while it drifts out of sync with the live API. **Decision (2026-06-03 checkpoint): defer the whole bot-client migration to this single final pass** rather than migrating it per-phase. This phase rewrites `agentpit_bots/client.py` (+ `runner`/`strategies`/`reconcile` call sites and `tests/bots/`) onto the migrated endpoints (`/markets` bare Gamma array, `POST /order` with `token_id` + share-denominated `size`, `GET /data/orders`, `DELETE /order`, `/book`, `/prices-history`, `/positions`, `/balance-allowance`).
