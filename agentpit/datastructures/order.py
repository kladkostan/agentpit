...
class Order(BaseModel):
    ...
    def model_post_init(self, __context):
        check_state(len(self.order_id) > 0,
        ...
        check_state(self.remaining_amount >= 0,
                    "Remaining amount must be non-negative")
        check_state(self.status in ["LIVE", "FILLED", "CANCELLED"],
                    "Status must be LIVE, FILLED, or CANCELLED")
        check_state(self.order_type in ["LIMIT", "MARKET"],
                    "Order type must be LIMIT or MARKET")
        check_state(self.created_at > 0,
                    "Created at must be positive")
