...
def test_resolve_market():
    with TestClient(main.server) as client:
        ...
        # Winner: Mint USDC and buy Yes tokens
        ...
        assert split_resp.status_code == 200
        assert split_resp.json()["token_balances"]["1"] == 100  # 100 Yes tokens
        assert split_resp.json()["token_balances"]["2"] == 100  # 100 No tokens

        # Loser: Mint USDC and buy No tokens (by splitting then selling Yes)
        ...
        # Winner redeems positions - should get USDC for Yes tokens
        ...
        assert redeem_winner_body["payout_usdc"] == 100  # 100 Yes tokens * 1 USDC each
        ...
        # Loser redeems positions - should get nothing (No tokens are worthless)
        ...
        assert redeem_loser_body["payout_usdc"] == 50  # 50 Yes tokens * 1 USDC each
        assert redeem_loser_body["tokens_redeemed"]["1"] == 50
        assert redeem_loser_body["tokens_redeemed"]["2"] == 50

        # Verify loser's USDC balance only increased by Yes tokens
...
def test_redeem_position_unresolved_market():
    with TestClient(main.server) as client:
        ...
        client.post(
            f"/markets/{market_id}/split_position",
            json={"api_key": api_key, "amount": 50}
        )
