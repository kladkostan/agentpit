...
describe("projectToViewBox", () => {
  ...
  it("uses an explicit fixedMax to focus the vertical domain", () => {
    // With the domain capped at 0.25 (25%), a 0.25 probability sits at the
    // top and 0.125 at the vertical center.
  ...
  it("clamps probabilities above fixedMax to the top edge", () => {
    const out = projectToViewBox(
      [{ t: 0, p: 0.9 }],
      { ...dims, fixedMax: 0.25 },
    );
  ...
  it("maps probability 1 to y=0 (top) and probability 0 to y=height (bottom)", () => {
    const out = projectToViewBox(
      [
        { t: 0, p: 0 },
        { t: 1, p: 1 },
  ...
  it("clamps probabilities outside [0, 1] to the chart edges", () => {
    const out = projectToViewBox(
      [
        { t: 0, p: -0.1 },
        { t: 1, p: 2 },
  ...
  it("relative scaleMode stretches the input probability range across the full height", () => {
    ...
    // the chart — same behavior the home-page Sparkline has.
    const out = projectToViewBox(
      [
        { t: 0, p: 0.45 },
        { t: 1, p: 0.5 },
    ...
    expect(out[0]![1]).toBe(50);
    expect(out[1]![1]).toBeCloseTo(25, 10);
